<template>
    <div class="location-list">
        <el-table :data="data" border>
            <el-table-column prop="first" label="一级区域" width="300" :filters="filters" :filter-method="filterFirst" filter-placement="right">
                <template slot-scope="scope">{{scope.row.first}}</template>
            </el-table-column>
            <el-table-column prop="second" label="二级区域"></el-table-column>
...
<script>
    export default {
        ...
        methods: {
            ...
            deleteLocation(id) {
                this.$confirm('此操作将删除二级区域且不可恢复，确认删除吗？', '提示', {
                    type: 'warning',
                    center: true
                }).then(() => {
